...
contract SvgNft is ERC721, Ownable {
    ...
    function mint() external payable returns (uint256) {
        if (msg.sender == address(0)) {
            revert Storage.Invalid_address();
        }

        uint256 tokenId = _nextTokenId++;
        _safeMint(msg.sender, tokenId);

        emit Storage.minted(msg.sender, tokenId);
    ...
    function token_url(uint256 tokenId) external view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked(
            '{"name":"Svg Time', tokenId.toString(), 
    ...
    function tokenExist(uint256 tokenId) internal view returns (bool) {
    ...
    function withdraw(uint256 tokenId) external onlyOwner {
        ...
        emit Storage.Withdrawn(address(this), tokenId);

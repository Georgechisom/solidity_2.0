...
async function main() {
  ...
  // approve
  const approveUsdcTx = await usdc_contract
    .connect(signer)
    .approve(Router_address, amountUsdcDesired);
  await approveUsdcTx.wait();

  const approveDaiTx = await dai_contract
    .connect(signer)
    .approve(Router_address, amountDaiDesired);
  await approveDaiTx.wait();

  try {
    const addLiquidityTx = await router_contract
      .connect(signer)
      .addLiquidity(
        usdc_address,
        Dai_address,
    ...
    console.error("Adding liquidity Successful:", addLiquidityTx);
  } catch (error) {
    console.error("Error adding liquidity:", error);
  }

  const afterUsdcImpersonatorBalance = await usdc_contract.balanceOf(
    impersonateAccount_address
  );
  const afterDaiImpersonatorBalance = await dai_contract.balanceOf(
    impersonateAccount_address
  );
  const afterUsdcImpersonatorPoolBalance = await usdc_contract.balanceOf(
    pool_address
  );
  const afterDaiImpersonatorPoolBalance = await dai_contract.balanceOf(
    pool_address
  );
  const afterPoolBalance = await pool_contract.balanceOf(
    impersonateAccount_address
  );
  ...
  console.log(
    "USDC Impersonator Balance:",
    ethers.formatEther(afterUsdcImpersonatorBalance)
  );
  console.log(
    "DAI Impersonator Balance:",
    ethers.formatEther(afterDaiImpersonatorBalance)
  );
  console.log(
    "USDC Impersonator Pool Balance:",
    ethers.formatEther(afterUsdcImpersonatorPoolBalance)
  );
  console.log(
    "DAI Impersonator Pool Balance:",
    ethers.formatEther(afterDaiImpersonatorPoolBalance)
  );
  console.log("Pool Balance:", ethers.formatEther(afterPoolBalance));
}
